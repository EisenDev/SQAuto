      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-slate-900/30 border border-slate-800 border-dashed rounded-3xl space-y-4">
            ...
            <button className="text-teal-500 text-sm font-bold hover:underline">Create your first organization</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {orgs.map((org) => (
              <div 
                key={org.id}
                onClick={() => router.push(`/projects/${org.id}`)}
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map(i => (
                      <div key={i} className="h-6 w-6 rounded-full bg-slate-800 border-2 border-slate-900 flex items-center justify-center text-[8px] font-bold text-slate-500">U{i}</div>
                    ))}
                  </div>
                </div>

                <h2 className="text-xl font-bold text-white group-hover:text-teal-300 transition-colors">{org.name}</h2>
                <p className="text-slate-500 text-sm mt-2 line-clamp-2 h-10">{org.description || "No description provided."}</p>
